...
def predict_all_texts(all_texts, true_names, training_files, verbose, validation, num_of_tokens_per_text):
    ...
    if validation:
        for i in range(0, len(all_texts)):
            average_accuracy += predict_text_by_comment(all_texts[i], true_names[i], training_files, verbose,
                                                        validation, num_of_tokens_per_text)
        print("Accuracy for Validation Set with ", num_of_tokens_per_text, "tokens per text: ",
              average_accuracy / len(all_texts), "%.")
    else:
        for i in range(0, len(all_texts)):
            average_accuracy += predict_text_by_comment(all_texts[i], test_author_information[true_names[i]],
                                                        training_files, verbose, validation, num_of_tokens_per_text)
        print("Accuracy for Test Set with ", num_of_tokens_per_text, "tokens per text: ",
              average_accuracy / len(all_texts), "%.")
            ...
            if verbose:
                if validation:
                    print("Most probable Author for Comment", text_list[comment], "is", training_files[best_index],
                          "with ", max_accuracy, "% accuracy, but true author is", true_name)
                else:
                    print("Most probable Author for Comment", text_list[comment], "is", training_files[best_index],
                          "with ",
...
while not input_correct:
    ...
    if input == 'v':
        ...
        predict_all_texts(validation_chain_list, onlyfiles, onlyfiles, verbose=False, validation=True,
                          num_of_tokens_per_text=tokens)
    elif input == 't':
        input_correct = True
        predict_all_texts(test_chain_list, onlyfiles_test, onlyfiles, verbose=False, validation=False,
                          num_of_tokens_per_text=tokens)
    elif input == 'o':
        input_correct = True
